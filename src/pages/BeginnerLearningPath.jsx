...
const BeginnerLearningPath = () => {
  ...
  const [showQuizSelection, setShowQuizSelection] = useState(false) // New: show quiz selection screen
  const [currentQuiz, setCurrentQuiz] = useState(null) // New: track which quiz is being taken
  ...
  const submitQuiz = () => {
    ...
    // For now, we'll mark it completed when any quiz is completed
    // You can enhance this later to require all quizzes
  ...
  const resetQuiz = () => {
    ...
    setShowQuizResults(false)
    setQuizScore(0)
    // Reset to quiz selection if multiple quizzes exist
    const quizzes = currentModule.quizzes || (currentModule.quiz ? [currentModule.quiz] : [])
    if (quizzes.length > 1) {
      setShowQuizSelection(true)
      setCurrentQuiz(null)
          ...
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                <BookOpen className="h-8 w-8 text-green-600 mr-3" />
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Beginner Learning Path</h1>
                  <p className="text-sm text-gray-600">Master the fundamentals of plants and soil</p>
                </div>
              </div>
            </div>
            <div className="text-sm text-gray-600">
